...
class CompanyController extends Controller
{
    ...
    public function tableClient(Request $request)
    {
        ...
        if (isset($type)) {
            $data = User::whereNotNull('parent_id')->where('role', 'client')->where('type', $type)->where('contact', 'like', '%' . $contact . '%')
                ->orderBy('created_at', 'desc')->get()->toArray();
        } else {
            $data = User::whereNotNull('parent_id')->where('role', 'client')->where('contact', 'like', '%' . $contact . '%')->orderBy('created_at', 'desc')->get()->toArray();
        }
        $arr = array();
