<table class="table table-separate table-head-custom table-checkable" id="table">
    ...
    @foreach($data as $index => $item)
        <tr style="{{$item['status'] == 0 ? 'color: red' : ''}}">
            ...
            <td class="p-0 border text-center align-middle">{{$item['this_month']}}</td>
            <td class="p-0 border text-center align-middle">{{$item['prev_month']}}</td>
            <td class="p-0 border text-center align-middle">{{$item['total_cnt']}}</td>
